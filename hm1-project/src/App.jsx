...
function App() {
	...
	const hundleOperator = (btnValue) => {
		...
		if (operand1 !== '' && operatorButton === '=' && operand2 !== '' && operator === '+') {
			setOperator(operator)
			setOperand1(Number(operand1) + Number(operand2))
			setOperand2('')
			setOperator('')
			setFlag(true)
		} else if (operand1 !== '' && operatorButton === '=' && operand2 !== '' && operator === '-') {
			setOperator(operator)
			setOperand1(Number(operand1) - Number(operand2))
			setOperand2('')
			setOperator('')
	...
	return (
		<>
			<h1>CALCULATOR</h1>
			<div className={`${flag ? styles.expressionResult : styles.expressionNotResult} ${styles.expression} `}>
				<output>{output}</output>
			</div>
			<div className={styles.calc}>
				<div className={styles.calculatorNum}>
					{nums.map((btn) => (
						<button onClick={() => hundleOperandButton(btn)} value={btn} key={btn}>{btn}</button>
					))}
				</div>
				<div className={styles.calculatorOp}>
					{operatBtn.map((opBtn) => (
						<button className={styles.opAndRes} onClick={() => hundleOperator(opBtn)} value={opBtn} key={opBtn+0}>{opBtn}</button>
					))}
				</div>
			</div>
		</>
