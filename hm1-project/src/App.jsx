		...
		<>
			<div className={styles.app}>
				...
				<div className={styles.listContainer}>
					...
					{ list.length === 0  ? <p className={styles.noMarginText}>Нет добавленных элементов</p> : <ul className={styles.list}>
						{list.map(({id, value, date}) => (<li className={styles.listItem} key={id}>{value} | {date}</li>))}
					</ul>
					}
				</div>
			</div>
